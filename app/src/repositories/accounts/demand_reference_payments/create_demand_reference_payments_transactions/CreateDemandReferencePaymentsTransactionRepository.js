...
import { createAxiosMultipartPostRequest } from "../../../../utils/api_client.js";
 ...
 */
export class CreateDemandReferencePaymentsTransactionRepository extends BaseRepository {
  /**
  ...
  constructor(payload = {}, retryAttempts = 1) {
    super({ endpoint: "create_demand_reference_payment_trx.php", payload, retryAttempts });
  }
  ...
  onDevise() {
    createAxiosMultipartPostRequest(this.endpoint, this.payload)
      .then(({ state, data }) => {
        if (state === "OK") {
          this._onSuccess(data.payment_timeline);
