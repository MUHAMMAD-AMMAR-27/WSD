...
import { createAxiosPostRequest } from "../../../utils/api_client.js";
 ...
 */
export class FetchAllApplicantsRepository extends BaseRepository {
  /**
  ...
  constructor(payload = {}, retryAttempts = 1) {
    super({ endpoint: "fetch_all_applicants.php", payload, retryAttempts });
  }
   ...
   */
  onDevise() {
    createAxiosPostRequest(this.endpoint, this.payload)
      .then(({ state, data }) => {
        if (state === "OK") {
          this._onSuccess(data.applicants);
