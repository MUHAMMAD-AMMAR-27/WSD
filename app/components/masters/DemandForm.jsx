...
const DemandForm = ({ ref, demandReferenceName, status, demandReferenceAddress, city, editMode, demandReferenceNumber, demandReferenceSecondaryNumber, country, demandReferencePassword }) => {
  ...
  const [demandReferenceEditMode, setDemandReferenceEditMode] = useState(null)

  useEffect(() => {
    if (editMode != null) {
      setReferenceName(demandReferenceName || "")
      setPhoneNumber(demandReferenceNumber || "")
      setSecondaryPhoneNumber(demandReferenceSecondaryNumber || "")
      setAddress(demandReferenceAddress || "")
      setResidentialCity(city || "")
      setPassword(demandReferencePassword || "")
      setIsCountry(country || "")
      setSelectedOption(status || "")
      setDemandReferenceEditMode(editMode)
  ...
  }, [editMode, demandReferenceName, demandReferenceNumber, demandReferenceSecondaryNumber, demandReferenceAddress, city, demandReferencePassword, country, status, countries])
  ...
  const addNewDemandReference = () => {
    ...
    fetch(createApiRoute("create_demand_reference.php"), {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
      },
      ...
      .then(res => {
        if (res.state === "FAILURE") {
          ...
          alert("Failed to add demand reference. Please try again.")
        } else {
          console.log(res)
          setMessageType("success")
          setShowSuccess(true)
  ...
  const updateDemandReference = () => {
    setIsLoading(true)

    const data = createDataForEevee({
      demand_reference_id: demandReferenceEditMode,
      reference_name: referenceName,
      phone_number: phoneNumber,
      secondary_phone_number: secondaryPhoneNumber,
      address: address,
    ...
    fetch(createApiRoute("update_demand_reference.php"), {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
      },
      ...
      .then(res => {
        if (res.state === "FAILURE") {
          setMessageType("failure")
          alert("Failed to update demand reference. Please try again.")
        } else {
          setMessageType("success")
          setShowSuccess(true)
          setTimeout(() => setShowSuccess(false), 4000)
  ...
  const handleSubmit = () => {
    if (demandReferenceEditMode) {
      updateDemandReference()
    } else {
      addNewDemandReference()
    }
  }
  ...
  return (
    <>
      ...
      <div className="text-sm text-gray-500 mb-4">
        <span className="font-medium text-gray-700">Dashboard</span> / Masters/ Add Demand Reference
      </div>
      <div className="bg-white rounded-xl shadow-md p-5 md:p-6 mb-8">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          {demandReferenceEditMode ? `Update ${ref} Reference` : `Add New ${ref} Reference`}
        </h2>
        ...
        <div className="flex gap-2">
          ...
          >
            <PlusIcon />
            {isLoading ? "Processing..." : demandReferenceEditMode ? `Update ${ref} Reference` : `Add ${ref} Reference`}
          </button>
        </div>
      </div>
    </>
