...
import { PencilIcon, Trash2 } from "lucide-react"
  ...
  const AddingNewDemandRef = ({onDemandReferenceEdit,referenceName, status, address, city, country, demandReferenceId, phoneNumber,secondaryPhoneNumber, password, onDeleteDemandReference}) => {
    ...
    <div className="space-y-3 p-2">
      ...
      shadow-lg duration-200 sm:max-w-lg border-none bg-white"
          ...
          dark:hover:bg-input/50 h-9 px-4 py-2 has-[>svg]:px-3 cursor-pointer border-gray-200"
              ...
              <button
                onClick={onDeleteDemandReference}
                data-slot="button"
                className="inline-flex items-center justify-center gap-2 whitespace-nowrap
          rounded-md text-sm font-medium transition-all disabled:pointer-events-none
          disabled:opacity-50 [&_svg]:pointer-events-none
        ...
        <div>
          ...
          <p className="text-xs text-gray-900">Code ID: {demandReferenceId}</p>
        </div>
        <div className="flex gap-1 md:mt-3 items-center">
          <button onClick={() => {
              onDemandReferenceEdit({
                referenceName,
                status,
                address,
                city,
...
export default AddingNewDemandRef;
